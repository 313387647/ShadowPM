...
import { Check, CircleDollarSign, Loader2, Pencil, Trash2, XCircle } from "lucide-react";
...
import { BudgetTaskRelationPicker } from "./BudgetTaskRelationPicker";
...
export function BudgetItemDrawer({ item, flows, tasks, canEdit, canManage, open, onOpenChange }: { item: BudgetItem | null; flows: Flow[]; tasks: TaskOption[]; canEdit: boolean; canManage: boolean; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  useEffect(() => { if (open) { setTab("overview"); setEditing(false); setSaveState("idle"); setShowCancel(false); setShowDelete(false); } }, [item?.id, open]);
  if (!item) return null;
  ...
  return <><Sheet open={open} onOpenChange={onOpenChange}><SheetContent size="full" className="max-w-2xl"><div className="shrink-0"><SheetHeader title={item.title} /><div className="flex items-center justify-between gap-3 border-b border-border px-5 py-2.5 sm:px-6"><Badge variant="outline">{STATUS_LABEL[item.status]}</Badge><div className="flex items-center gap-1">{canEdit && tab === "overview" && <Button type="button" size="sm" variant="ghost" className="h-8 gap-1.5" onClick={() => { setEditing((value) => !value); setSaveState("idle"); }}><Pencil className="size-3.5" />{editing ? "取消" : "编辑"}</Button>}{canManage && item.status === "DRAFT" && <form onSubmit={(event) => { event.preventDefault(); const formData = new FormData(event.currentTarget); formData.set("status", "CONFIRMED"); void saveItem(formData); }}>{staticFields}<Button type="submit" size="sm" disabled={submitting} className="gap-1.5"><Check className="size-3.5" />确认</Button></form>}{canManage && item.status !== "DRAFT" && item.status !== "CANCELED" && <Button type="button" size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => setShowCancel(true)}>取消预算项</Button>}{canEdit && item.status === "DRAFT" && <Button type="button" size="icon" variant="ghost" className="size-8 text-muted-foreground hover:text-destructive" title="删除草稿预算项" onClick={() => setShowDelete(true)}><Trash2 className="size-3.5" /></Button>}</div></div><div className="grid grid-cols-3 divide-x divide-border border-b border-border text-center"><Metric label="计划金额" value={formatBudgetMoney(item.plannedAmount)} /><Metric label="实际支出" value={formatBudgetMoney(item.actualSpend)} /><Metric label="剩余" value={formatBudgetMoney(item.remaining)} /></div><div className="flex gap-5 border-b border-border px-5 sm:px-6">{([ ["overview", "概况"], ["actions", "资金动作"], ["history", "历史"] ] as const).map(([key, label]) => <Button key={key} type="button" variant="ghost" onClick={() => { setTab(key); setEditing(false); }} className={tab === key ? "h-10 rounded-none border-b-2 border-primary px-0 text-primary hover:bg-transparent hover:text-primary" : "h-10 rounded-none px-0 text-muted-foreground hover:bg-transparent hover:text-foreground"}>{label}</Button>)}</div></div>
    <div className="min-h-0 flex-1 overflow-y-auto">{tab === "overview" && <Overview item={item} tasks={tasks} canEdit={canEdit} editing={editing} submitting={submitting} onSave={saveItem} />}{tab === "actions" && <Actions item={item} canEdit={canEdit} flowAction={flowAction} setFlowAction={setFlowAction} submitting={submitting} onSave={saveFlow} />}{tab === "history" && <History item={item} flows={itemFlows} />}{saveState === "saved" && <p className="px-5 pb-4 text-xs text-success sm:px-6">已保存</p>}</div>
...
function Overview({ item, tasks, canEdit, editing, submitting, onSave }: { item: BudgetItem; tasks: TaskOption[]; canEdit: boolean; editing: boolean; submitting: boolean; onSave: (formData: FormData) => Promise<void> }) {
  const [plannedAmount, setPlannedAmount] = useState(String(item.plannedAmount));
  useEffect(() => setPlannedAmount(String(item.plannedAmount)), [item.id, item.plannedAmount]);
  if (!canEdit || item.status === "CANCELED" || !editing) return <div className="space-y-5 px-5 py-5 text-sm sm:px-6"><Field label="分类" value={item.category ?? "未分类"} /><Field label="关联事项" value={item.taskNames.length ? item.taskNames.join("、") : "暂未关联"} /><Field label="说明" value={item.description ?? "暂无说明"} /></div>;
  const requiresReason = item.status !== "DRAFT" && Number(plannedAmount) !== item.plannedAmount;
  return <form className="space-y-4 px-5 py-5 sm:px-6" onSubmit={(event) => { event.preventDefault(); void onSave(new FormData(event.currentTarget)); }}><input type="hidden" name="budgetItemId" value={item.id} /><input type="hidden" name="status" value={item.status} /><div className="grid gap-3 sm:grid-cols-2"><FieldInput label="预算项名称" name="title" value={item.title} required /><label className="space-y-1"><span className="text-xs text-muted-foreground">计划金额</span><Input name="plannedAmount" type="number" min="0.01" step="0.01" value={plannedAmount} onChange={(event) => setPlannedAmount(event.target.value)} required className="h-9" /></label><FieldInput label="分类" name="category" value={item.category ?? ""} />{requiresReason && <FieldInput label="修改原因" name="reason" value="" required placeholder="请说明调整已确认预算的原因" />}</div><label className="block space-y-1"><span className="text-xs text-muted-foreground">说明</span><Textarea name="description" defaultValue={item.description ?? ""} rows={3} className="resize-none" /></label><BudgetTaskRelationPicker tasks={tasks} initialTaskIds={item.taskIds} /><div className="flex justify-end"><Button type="submit" size="sm" disabled={submitting} className="gap-1.5">{submitting && <Loader2 className="size-3.5 animate-spin" />}保存</Button></div></form>;
}
...
function Actions({ item, canEdit, flowAction, setFlowAction, submitting, onSave }: { item: BudgetItem; canEdit: boolean; flowAction: string; setFlowAction: (value: string) => void; submitting: boolean; onSave: (formData: FormData) => Promise<void> }) {
  ...
  return <form className="space-y-4 px-5 py-5 sm:px-6" onSubmit={(event) => { event.preventDefault(); void onSave(new FormData(event.currentTarget)); }}><div className="flex items-center gap-2 text-sm font-medium"><CircleDollarSign className="size-4 text-primary" />记录资金动作</div><input type="hidden" name="budgetItemId" value={item.id} /><div className="grid gap-3 sm:grid-cols-2"><label className="space-y-1"><span className="text-xs text-muted-foreground">动作</span><Select name="action" value={flowAction} onChange={(event) => setFlowAction(event.target.value)}><option value="APPROVAL_RECORDED">报批通过</option><option value="TRANSFER_RECORDED">划拨给第三方</option><option value="EXPENSE_RECORDED">记录实际支出</option><option value="REFUND_RECORDED">记录退款</option><option value="SETTLED">验收并结算</option><option value="REVERSED">撤回上一动作</option></Select></label><FieldInput label="金额" name="amount" value="" type="number" required />{flowAction === "TRANSFER_RECORDED" && <div className="sm:col-span-2"><FieldInput label="接收部门或合作方" name="counterparty" value="" required /></div>}</div><label className="block space-y-1"><span className="text-xs text-muted-foreground">说明</span><Textarea name="description" required rows={3} placeholder="例如：供应商首付款已审批并支付" className="resize-none" /></label><div className="flex justify-end"><Button type="submit" size="sm" disabled={submitting} className="gap-1.5">{submitting && <Loader2 className="size-3.5 animate-spin" />}记录动作</Button></div></form>;
}
