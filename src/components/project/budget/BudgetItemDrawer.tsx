...
export function BudgetItemDrawer({ item, flows, tasks, canEdit, canManage, open, onOpenChange }: { item: BudgetItem | null; flows: Flow[]; tasks: TaskOption[]; canEdit: boolean; canManage: boolean; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent className="max-w-2xl"><div className="p-5 sm:p-6"><SheetHeader title={item.title} description="预算项独立存在，关联事项只补充工作上下文。" /><div className="mt-3"><Badge variant="outline">{STATUS_LABEL[item.status]}</Badge></div>
