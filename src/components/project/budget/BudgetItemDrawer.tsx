...
function Overview({ item, tasks, canEdit, editing, submitting, onSave }: { item: BudgetItem; tasks: TaskOption[]; canEdit: boolean; editing: boolean; submitting: boolean; onSave: (formData: FormData) => Promise<void> }) {
  ...
  return <form className="space-y-4 px-5 py-5 sm:px-6" onSubmit={(event) => { event.preventDefault(); void onSave(new FormData(event.currentTarget)); }}><input type="hidden" name="budgetItemId" value={item.id} /><input type="hidden" name="status" value={item.status} /><div className="grid gap-3 sm:grid-cols-2"><FieldInput label="预算项名称" name="title" value={item.title} required /><FieldInput label="计划金额" name="plannedAmount" value={item.plannedAmount} type="number" required /><FieldInput label="分类" name="category" value={item.category ?? ""} /><FieldInput label="修改原因" name="reason" value="" placeholder={item.status === "DRAFT" ? "草稿可不填" : "调整已确认金额时必填"} /></div><label className="block space-y-1"><span className="text-xs text-muted-foreground">说明</span><Textarea name="description" defaultValue={item.description ?? ""} rows={3} className="resize-none" /></label><fieldset><legend className="text-xs text-muted-foreground">关联管控事项（可不选、可多选）</legend><div className="mt-2 max-h-36 divide-y divide-border overflow-y-auto border-y border-border">{tasks.map((task) => <label key={task.id} className="flex min-h-10 items-center gap-2 py-2 text-sm"><input type="checkbox" name="taskIds" value={task.id} defaultChecked={item.taskIds.includes(task.id)} />{task.name}</label>)}</div></fieldset><div className="flex justify-end"><Button type="submit" size="sm" disabled={submitting} className="gap-1.5">{submitting && <Loader2 className="size-3.5 animate-spin" />}保存</Button></div></form>;
}
