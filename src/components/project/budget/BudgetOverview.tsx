...
export function BudgetOverview({ totalBudget, planned, remainingToAllocate, actualSpend }: { totalBudget: number; planned: number; remainingToAllocate: number; actualSpend: number }) {
  ...
  return <section className="border-y border-border py-4"><div className="flex flex-wrap items-baseline gap-x-3 gap-y-1"><p className="text-sm text-muted-foreground">项目总预算</p><p className="font-mono text-2xl font-semibold tracking-tight tabular-nums">{formatBudgetMoney(totalBudget)}</p><p className="text-sm text-muted-foreground">已编排 <span className="font-mono text-foreground tabular-nums">{formatBudgetMoney(planned)}</span> · 实际支出 <span className="font-mono text-foreground tabular-nums">{formatBudgetMoney(actualSpend)}</span> · <span className={overAllocated ? "text-destructive" : "text-muted-foreground"}>剩余 <span className="font-mono tabular-nums">{formatBudgetMoney(remainingToAllocate)}</span></span></p></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><ProgressLine label="预算编排" value={planned} total={totalBudget} tone="primary" /><ProgressLine label="实际支出" value={actualSpend} total={totalBudget} tone="success" /></div></section>;
}
