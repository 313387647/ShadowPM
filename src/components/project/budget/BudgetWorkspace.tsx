...
export function BudgetWorkspace({ data, tasks, canEdit, canManage }: { data: Planning; tasks: TaskOption[]; canEdit: boolean; canManage: boolean }) {
  ...
  return <section className="space-y-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="section-kicker">Budget Workspace</p><p className="mt-1 text-sm text-muted-foreground">先确认预算池，再编排预算项；每一笔资金动作独立留痕。</p></div>{canManage && <BudgetPoolEditor projectId={data.projectId} mode={data.pool.mode} totalBudget={data.pool.totalBudget} triggerLabel="管理总预算" />}</div><BudgetOverview totalBudget={data.pool.totalBudget} planned={data.pool.planned} remainingToAllocate={data.pool.remainingToAllocate} actualSpend={data.pool.actualSpend} />
