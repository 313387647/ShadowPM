...
import { Landmark, ListTree, ReceiptText } from "lucide-react";
...
export function BudgetWorkspace({ data, tasks, canEdit, canManage }: { data: Planning; tasks: TaskOption[]; canEdit: boolean; canManage: boolean }) {
  ...
  return <section className="space-y-4"><div className="flex flex-wrap items-end justify-between gap-3 border-b border-border pb-3"><div><h2 className="text-base font-semibold">预算</h2><p className="mt-1 text-xs text-muted-foreground">项目总预算限定规划上限；资金动作在预算项下独立留痕。</p></div>{canManage && <BudgetPoolEditor projectId={data.projectId} mode={data.pool.mode} totalBudget={data.pool.totalBudget} triggerLabel="管理总预算" />}</div><BudgetOverview totalBudget={data.pool.totalBudget} planned={data.pool.planned} remainingToAllocate={data.pool.remainingToAllocate} actualSpend={data.pool.actualSpend} />
    ...
    <div className="inline-flex rounded-lg border border-border bg-canvas/35 p-1"><Button type="button" variant="ghost" onClick={() => setView("planning")} className={view === "planning" ? "h-8 bg-surface-3 text-foreground shadow-sm hover:bg-surface-3" : "h-8 text-muted-foreground"}><ListTree className="mr-1.5 size-3.5" />预算规划</Button><Button type="button" variant="ghost" onClick={() => setView("flows")} className={view === "flows" ? "h-8 bg-surface-3 text-foreground shadow-sm hover:bg-surface-3" : "h-8 text-muted-foreground"}><ReceiptText className="mr-1.5 size-3.5" />资金流水</Button></div>
