...
function ControlItemDetailSheet({ task, phases, canEdit, onOpenChange, onDelete }: { task: Task | null; phases: PhaseOption[]; canEdit: boolean; onOpenChange: (open: boolean) => void; onDelete: () => void }) {
  const router = useRouter();
  ...
  async function recordProgress() {
    if (!progress.trim() || savingProgress) return;
    setSavingProgress(true);
    const formData = new FormData();
    formData.set("taskId", currentTask.id);
    formData.set("content", progress.trim());
    formData.set("syncTaskNotes", "on");
    try {
      const result = await addProgressLog(formData);
      if (!result.success) toast.error(result.message ?? "记录失败");
      else { setProgress(""); router.refresh(); }
    } catch {
      toast.error("记录失败");
    } finally {
  ...
  async function changeStatus(status: Task["status"]) {
    if (!canEdit || status === currentTask.status) return;
    const result = await updateTaskStatus(currentTask.id, status);
    if (!result.success) toast.error(result.message ?? "状态更新失败");
    else router.refresh();
  }
