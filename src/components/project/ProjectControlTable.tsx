...
function ControlTableRow({ task, phases, canEdit, onOpen }: { task: Task; phases: PhaseOption[]; canEdit: boolean; onOpen: () => void }) {
  ...
  const showAiSignal = Boolean(task.sourceRef && (task.needsConfirmation || (task.aiConfidence && task.aiConfidence !== "HIGH")));
  return <tr className="group cursor-pointer transition-colors hover:bg-primary/[0.04]" onClick={onOpen}>
    <td className="px-4 py-3"><div className="min-w-0"><div className="mb-1 flex flex-wrap gap-1.5">{phaseName && <span className="rounded border border-border bg-secondary px-1.5 py-0.5 text-[10px] text-muted-foreground">{phaseName}</span>}{showAiSignal && <span className="rounded border border-primary/20 bg-primary/[0.07] px-1.5 py-0.5 text-[10px] text-primary">AI 待确认</span>}</div><p className="truncate font-medium">{task.name}</p>{task.description && <p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{task.description}</p>}</div></td>
...
function ControlItemSignals({ task }: { task: Task }) { const signals = [isOverdue(task) ? "已逾期" : null, !task.assignee ? "待补负责人" : null, !task.deadline ? "待补日期" : null, task.aiConfidence && task.aiConfidence !== "HIGH" ? "AI 低置信" : null, task.needsConfirmation ? "待确认" : null].filter((item): item is string => Boolean(item)); if (!signals.length) return null; const primary = signals[0]; return <span className={primary === "已逾期" ? "rounded border border-destructive/25 bg-destructive/10 px-1.5 py-0.5 text-[10px] text-destructive" : "rounded border border-warning/25 bg-warning/10 px-1.5 py-0.5 text-[10px] text-warning"}>{primary}{signals.length > 1 ? ` +${signals.length - 1}` : ""}</span>; }
