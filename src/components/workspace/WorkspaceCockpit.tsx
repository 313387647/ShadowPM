...
function StatusMarker({ overdue, status }: { overdue: boolean; status: string }) { const label = overdue ? "已逾期" : status === "IN_PROGRESS" ? "进行中" : status === "COMPLETED" ? "已完成" : "待启动"; const tone = overdue ? "bg-destructive" : status === "IN_PROGRESS" ? "bg-primary" : status === "COMPLETED" ? "bg-success" : "bg-muted-foreground"; return <span role="img" aria-label={label} title={label} className="flex size-5 shrink-0 items-center justify-center"><span className={`size-2 rounded-full ${tone}`} /></span>; }
