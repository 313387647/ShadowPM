  ...
  const project = await prisma.$transaction(async (tx) => {
    const createdProject = await tx.project.create({
      data: {
        ...
        budgetStatus: dto.createBudgetFlow && confirmedTotalBudget.gt(0) ? "CONFIRMED" : "UNCONFIRMED",
        ownerId: user.id,
    ...
    // The total budget is a project-level pool, never a disguised task allocation.
    if (dto.createBudgetFlow && confirmedTotalBudget.gt(0)) {
    ...
    let allocatedBudget = new Prisma.Decimal(0);
    const taskBudgetById = new Map<string, Prisma.Decimal>();
    for (const item of confirmedBudgetItems) {
      const taskId = findRelatedTaskId(item, createdTasks, firstTaskId);
      if (!dto.createBudgetFlow || !taskId || typeof item.amount !== "number") continue;
      const itemAmount = new Prisma.Decimal(item.amount.toString());
      if (allocatedBudget.add(itemAmount).gt(confirmedTotalBudget)) {
        budgetCandidates.push(item);
        continue;
      }

      const nextTaskAmount = (taskBudgetById.get(taskId) ?? new Prisma.Decimal(0)).add(itemAmount);
      ...
      await tx.task.update({
        where: { id: taskId },
        data: { budgetAmount: nextTaskAmount, budgetStatus: "ALLOCATED", budgetRecipient: null },
      });
      allocatedBudget = allocatedBudget.add(itemAmount);
      taskBudgetById.set(taskId, nextTaskAmount);
    ...
    await tx.activityLog.create({
      data: {
        ...
        summary: [
          ...
          dto.createBudgetFlow && confirmedTotalBudget.gt(0) ? `预算池：¥${confirmedTotalBudget.toNumber().toLocaleString("zh-CN")}` : null,
          createdBudgetFlows.length > 0 ? `事项预算分配：${createdBudgetFlows.length} 条` : null,
          budgetCandidates.length > 0 ? `未直接入账的预算线索：${budgetCandidates.length} 条` : null,
