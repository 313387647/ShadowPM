...
3. "风险及待确定项/问题/阻塞/待确认"通常应进入 risks。
...
10. tasks最多30条，budgetItems最多30条，calendarEntries最多30条，risks最多20条。
11. 不要编造缺失信息。缺失就返回null或空数组。
12. 只返回 JSON，不要 markdown。
13. 返回紧凑 JSON。不要解释，不要缩进，不要重复字段，不要输出 source 原文。
14. 如果源表很大，优先保留最能代表项目管控、预算、执行日历、风险的高价值条目，避免输出被截断。
...
- risks 最多 8 条，优先保留待确认、阻塞、权限、审批、排期类风险。
- 字段值未知就省略该字段或填 null。
...
export async function createProjectFromAI(
  ...
  const hasImportCandidates =
    ...
    importCandidates.risks.length > 0;
  ...
  const project = await prisma.$transaction(async (tx) => {
    ...
    await tx.assetFolder.createMany({
      data: PROJECT_DEFAULT_FOLDERS.map((name) => ({
...
function validateParsedProject(parsed: AIParsedProject): AIParsedProject {
  ...
  return {
    ...
    risks: (Array.isArray(parsed.risks) ? parsed.risks : [])
      .filter((risk) => risk?.title?.trim())
      .map((risk) => ({
        title: risk.title.trim(),
        description: risk.description?.trim() || null,
        type: risk.type?.trim() || null,
        level: risk.level?.trim() || null,
        status: risk.status?.trim() || null,
        owner: risk.owner?.trim() || null,
        relatedItemName: risk.relatedItemName?.trim() || null,
      }))
      .slice(0, 20),
